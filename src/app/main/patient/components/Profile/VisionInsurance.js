...
class VisionInsurance extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    console.log("profile++++++++++++++++++", this.props.getInsurName)
    return (
      <Card className="mb-16 w-full">
        <CardContent>
          <Typography variant="subtitle1" className="mb-6">Vision Insurance</Typography>
...
VisionInsurance = reduxForm({
  form: 'VisionInsuranceForm',
  enableReinitialize: true,
  destroyOnUnmount: true,
  onChange: (values, dispatch, { autoSave, pristine, dirty }) => {
    console.log('values ====> ',values);
    if (!pristine || dirty) {
      autoSave('visionInsurance', { visionInsurance: values });
    }
  }
})(VisionInsurance);
...
function mapStateToProps({ patientReducer }) {
  console.log(" patientReducer.profile", patientReducer.profile)
  return {
    initialValues: patientReducer.profile.visionInsurance,
...
export default withReducer('Reducer', reducer)(
  withStyles(styles, { withTheme: true })(
    withRouter(connect(mapStateToProps,mapDispatchToProps)(VisionInsurance))
  )
);
