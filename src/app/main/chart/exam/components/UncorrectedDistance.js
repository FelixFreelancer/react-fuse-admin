...
class UncorrectedDistance extends Component {
  render() {
    return (
        ...
        <div className="w-full md:w-1/2">
          <div className="mr-16">
            <Typography color="textSecondary" gutterBottom>
              Uncorrected Distance Visual Accuity
            </Typography>
            <Divider />
...
function mapStateToProps({ applicationsReducer }) {
  return {
    ...
    initialValues: applicationsReducer.exam.uncorrectedDistanceVa
  };
}

UncorrectedDistance = reduxForm({
  form: 'UncorrectedDistanceForm',
  enableReinitialize: true,
  destroyOnUnmount: true,
  onChange: (values, dispatch, props) => {
    if (!props.pristine || props.dirty) {
      props.saveExam('uncorrectedDistanceVa', props.chart._id, values);
    }
  }
})(UncorrectedDistance);
