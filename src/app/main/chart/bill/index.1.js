...
const renderSelectField = ({
  ...
  ...custom
    ...
    <FormControl error={touched && error}>
      ...
      <InputLabel htmlFor="CPT">{label}</InputLabel>
      <Select
        style={{ width: "100%" }}
        native
        {...input}
        ...
        options={items}
        isMulti={true}
        onChange={(event, value) => input.onChange((value.option && value.option.value) ? value.option.value : "")}
...
class Bill extends Component {
  ...
  handleChange = value => {
    ...
    console.log('this.props.formStates ====> ', this.props.formStates);
    const { pristine, dirty, chart: { _id }, saveBill,
      formStates:
      {
        isPaid,
        amount,
    ...
    let valueObj = {
      ...
      isBilledInsurance: isBilledInsurance,
      confirmationCode: confirmationCode,
    }
    console.log('multi ====> ', multi);
    if (!pristine || dirty) {
      saveBill(valueObj, _id);
    }
...
const mapStateToProps = (state) => {
  ...
  return {
    formStates: getFormValues('BillForm')(state),
